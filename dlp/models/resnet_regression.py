...
class PreResNet(nn.Module):
    def __init__(self, num_classes=1, depth=110, dropout_rate=0.0):
        ...
        self.hidden1 = nn.Linear(64 * block.expansion, 64) # My adjustments for linear regression start here
    ...
    def forward(self, x):
        ...
        x = self.hidden1(x) # extra steps added by me
        x = self.relu(x)
        x = self.hidden2(x)
        x = self.relu(x)
        x = self.fc(x)
